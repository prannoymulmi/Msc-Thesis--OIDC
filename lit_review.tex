...
\hline
Ownership & Owned and operated by the organisation or a third party dedicated to the organisation. & Operated by CSPs at their data centers. & Ownership is shared between private and public components. & Owned and operated by participating organisations or is dedicated to the community. \\
\hline
...
The valuable tokens can be intercepted using different attacks, like MitM and XSS attacks, allowing the valid tokens to be replayed.
...
Given the benefits of this flow, this design is still not without potential risks, namely misconfigured URLs, Cross-Site Request Forgery (CSRF) and also MitM attacks. 
...
\textbf{Tokens Exposed} & Yes, tokens are exposed in the URL, cookies, or client's local storage. & No, tokens are exchanged server-side. & No, it is the same as the authorisation code, but with PKCE parameters providing additional security. \\ \hline
...
Cloud environments typically share infrastructure, increasing the risk of data leakage and unauthorised access between tenants.
