...
Listing \ref{lst:chisqrd} shows the function used to calculate the chi-square and the p-value for the given samples. Using the function below, a \textbf{p-value} of 0.98 is calculated, which leads to the results not rejecting the null hypothesis and, therefore, shows that the chances of information leakage due to timing attacks are low and can mitigate such side-channel attacks.
...
\begin{longtable}{|c|c|}
\caption{Truncated timing samples for the hash comparison for incorrect password}
...
\begin{longtable}{|c|c|}
\caption{Truncated timing samples for the hash comparison for correct password}
